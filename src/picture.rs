...
enum PictureBuf {
    ...
    Rgb16 {
        buf: ImageBuffer<Rgb<u16>, Vec<u16>>,
    },
}
...
impl Picture {
    ...
    pub fn new(
        ...
        if aspect_ratio <= 0.0 {
            return Err(PictureErr::InvalidArgs {
                err: "aspect_ratio less or equal to 0 on creation of Picture".to_string(),
            });
        }

        if width == 0 || height == 0 || samples_per_pixel == 0 {
            return Err(PictureErr::InvalidArgs {
                err: "image width, height, or samples_per_pixel invalid on creation of Picture"
                    .to_string(),
            });
        }
        ...
        let img = match outtype {
            ...
            PictureType::Rgb16 => PictureBuf::Rgb16 {
                buf: ImageBuffer::new(width, height),
            },
        };
    ...
    pub fn write_row(&mut self, row: &[Color], row_num: u32) -> Result<(), PictureErr> {
        ...
        if row_num >= self.height() {
            return Err(PictureErr::InvalidArgs {
                err: "Row beyond height of image".to_string(),
            });
        }
        ...
        match &mut self.img {
            ...
            PictureBuf::Ppm {
                ...
                if *x != 0 || *y != row_num {
                    return Err(PictureErr::InvalidArgs {
                        err: "Ppm file not at correct coordinates at start of write_row"
                            .to_string(),
                    });
                }
    ...
    pub fn write_pixel(&mut self, pixel: &Color, row: u32, col: u32) -> Result<(), PictureErr> {
        if col >= self.width {
            return Err(PictureErr::InvalidArgs {
                err: "Column beyond width of image".to_string(),
            });
        }
        ...
        match &mut self.img {
            PictureBuf::Ppm {
                ...
                if *x != col || *y != row {
                    return Err(PictureErr::InvalidArgs {
                        err: "Ppm file not at correct coordinates at start of write_pixel"
                            .to_string(),
                    });
                }
